...
public class CustomDialogSocio {

	private Context contexto;
	private String res = "";
	private SocioNegocioBean sn = null;
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				if(position == 0){
					...
					if(aprobar){
						...
						sn = select.obtenerSocioNegocio(key);
						sn.setTransaccionMovil("4");
...
//						
//						Select select = new Select(context);
//						sn = new SocioNegocioBean();
//						sn = select.obtenerSocioNegocio(key);
//						sn.setTransaccionMovil("2");
//						select.close();
//						
//						 if(wifi || movil && isConnectionFast){
//					        	
...
//					    	 
//					     }else{
//					    	 Toast.makeText(contexto, "No est� conectado a ninguna red de datos", Toast.LENGTH_SHORT).show();
//					    	 dialog.dismiss();
//					     }
//					}else{
//						 Toast.makeText(contexto, "No tiene permisos para realizar esta acci�n", Toast.LENGTH_SHORT).show();
//					}
//					
//				}else if(position == 1){
//					
...
//				}
	...
	private class TareaActualizarOrd extends AsyncTask<String, Void, Object>{
		...
		protected Object doInBackground(String... arg0) {
			...
			res = ws.AgregarSocioNegocio(sn, sn.getContactos(), sn.getDirecciones());
		...
		protected void onPostExecute(Object result) {
			...
			if (res == null || res.equalsIgnoreCase("anytype{}")) {
				...
				insert.updateEstadoSocioNegocio(sn.getClaveMovil());
				insert.updateEstadoTransaccionSocioNegocio(sn.getClaveMovil(), 
						sn.getTransaccionMovil());
				insert.close();
