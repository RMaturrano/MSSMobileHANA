...
public class CustomDialogPago {

	private Context contexto;
	private String res = "";
	private PagoBean pg = null;
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				if(position == 0 ){
					...
					if(actualizar){
						...
						pg = new PagoBean();
						pg = select.obtenerPagoRecibido(key);
						pg.setTransaccionMovil("2");
						select.close();
	...
	private class TareaActualizarOrd extends AsyncTask<String, Void, Object>{
		...
		protected void onPostExecute(Object result) {
			...
			if (res == null || res.equalsIgnoreCase("anytype{}")) {
				...
				insert.updateEstadoPagoRecibido(pg.getClaveMovil());
				insert.updateEstadoTransaccionPagoRecibido(pg.getClaveMovil(), 
						pg.getTransaccionMovil());
				insert.close();
