...
public class DetalleVentaLinea extends AppCompatActivity{
	...
	protected void onCreate(@Nullable Bundle savedInstanceState) {
		...
		if (myIntent.getStringExtra("idOv") != null) {
			idDetalle = myIntent.getStringExtra("idOv");
		}
...
//		SQLiteDatabase db = cn.getWritableDatabase();
		...
		Cursor rs = db.rawQuery("select A.Nombre,U.Nombre, AL.NOMBRE," +
								...
								+ "from TB_ORDEN_VENTA_DETALLE D LEFT JOIN TB_ARTICULO A "
								+ "ON D.Articulo = A.Codigo LEFT JOIN TB_UNIDAD_MEDIDA U "
								+ "ON D.UnidadMedida = U.Codigo LEFT JOIN TB_ALMACEN AL "
								+ "ON D.Almacen = AL.CODIGO LEFT LEFT JOIN TB_LISTA_PRECIO LP "
								+ "ON D.ListaPrecio = LP.Codigo LEFT JOIN TB_IMPUESTO I "
								+ "ON D.Impuesto = I.Codigo " 
								+ "WHERE ClaveMovil = '"+idDetalle+"' " 
